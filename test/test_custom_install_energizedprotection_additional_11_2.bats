...
setup() {
	...
	# 0=Downloads the additional package from this repository website.
	# 1=Unpacks the downloaded additional_package zip
	# 2=Checks if the additional package is in the target file and export the true-false boolean to a file.
	...
	# 6=Export the remaining lines of the target file after the lines uptill the start line of additional package are removed.
	# 7=Creates a new output target file that contains the remainder after the detected starting line position.
	# 8=Outputs the absolute line end nr of the additional package in the original host file.
	# 9=Export a boolean indicating whether the modified additional package is contained.
	# 10=If modified additional package is found in the input  path, remove it.
	# 11=Appends the new additional pack if the unmodified additional package is not yet included.
	...
	declare -a additional_arguments=(
		"test/testfiles/dummy_hostfile_with_modified_additional_pack.txt"
	)

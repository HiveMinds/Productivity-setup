...
if [ $BUILD_LINUX == 1 ]; then
	...
	for arch in "x86_64"; do
		...
		# Add additional addon
		cp -RH "$CALLDIR/modules/zotero-scihub" "$APPDIR/extensions/scihub@example.com"
		perl -pi -e 's/\.SOURCE<\/em:version>/.SA.'"$VERSION"'<\/em:version>/' "$APPDIR/extensions/scihub@example.com/install.rdf"
		echo
		echo -n "scihub@example.com Version: "
		perl -ne 'print and last if s/.*<em:version>(.*)<\/em:version>.*/\1/;' "$APPDIR/extensions/scihub@example.com/install.rdf"
		echo
		###rm -rf "$APPDIR/extensions/scihub@example.com/.git"
		# Default preferenes are no longer read from built-in extensions in Firefox 60
		echo >> "$APPDIR/defaults/preferences/prefs.js"
		###cat "$CALLDIR/modules/zotero-scihub/defaults/preferences/zoteroOpenOfficeIntegration.js" >> "$APPDIR/defaults/preferences/prefs.js"
		echo 'pref("extensions.scihub@example.com.unopkgPaths", "{}");'  >> "$APPDIR/defaults/preferences/prefs.js"
